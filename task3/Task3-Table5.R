...
## third is for the two probabilities
est.probs <- array(dim = c(B, 3, 3, 3))
## first dim is the number of bootstraps samples,
## second is for the 3-folds,
## third is for the three method (HT, HRV, geometric) and
...
load("../outputs/Task3_crossvalidation.RData") ### in the paper


logscore <- array(dim = c(B,3,3))

# Compute Brier and logarithmic scores
for(b in 1:B){
  for(i in 1:3){ # method (HT, HRV, geometric)
    for(j in 1:3){
        logscore[b,i,j] <- mean(bin.holds[b,,1,min(j,2)]*log(1-est.probs[b,,i,j]) + # {#0} * log(1-p)
                                 bin.holds[b,,2,min(j,2)]*log(est.probs[b,,i,j])) # {#1} * log(p)
    }
  }
}
...
mlogscore <- apply(logscore, 2:3, mean)
# Standard error (not independent, but...)
apply(logscore, 2:3, sd)/sqrt(3)
options(knitr.kable.NA = '')
row.names(mlogscore) <- c("conditional","HRV","geometric")
cat(
knitr::kable(apply(mlogscore, 1:2, function(x){
  ifelse(is.na(x), NA_character_, paste0("$", sprintf(fmt = "%.2f", x), "$"))}), 
