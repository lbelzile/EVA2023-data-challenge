...
## This code contains functions from Sam Perreault
## Sadly, some of the code is in a private Github repository
# remotes::install_github("samperochkin/tautest")
# library(tautests)
# New repository samperochkin/taujack contains other functions
# But not in a R package, so need some hack to get the code
## Functions not found on Github repository 'taujack'
r2ij <- function(r){
...
R2IJ <- function(R){
   return(t(sapply(R, r2ij)))
}

# Other functions by Sam Perreault on Github (but not in a package!)


loadCpp <- function(remurl){
   locfile <- "/tmp/file.cpp"
   download.file(url=remurl, destfile=locfile)
   Rcpp::sourceCpp(locfile)
}
loadCpp("https://raw.githubusercontent.com/samperochkin/taujack/main/src/ms.cpp")
source("https://raw.githubusercontent.com/samperochkin/taujack/main/functions.R")
...
# Th <- TSh$tau
TSh <- taujack_ms(X)
# TSh$Th contains vectorized Kendall tau
Th <- pcaPP::cor.fk(X)
Sh <- TSh$Sh
...
abline(v = loss_E,
       ...
       lty = 3,
       lwd = 2)
# p-values (asymptotic and Monte Carlo)
c(pval_E, mean(loss_E <= mc_loss_E))
...
abline(v = loss_M,
       ...
       lty = 3,
       lwd = 2)
# Report p-values for two statistics
c(pval_E, pval_M)

# Case for the I/n as scaling factor
St_dag <- (IG %*% St %*% IG)

# p-values via Monte Carlo
mc_z <- mvtnorm::rmvnorm(1e4, sigma = St_dag)
