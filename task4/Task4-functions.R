...
texmex_ConstraintsAreSatisfied <- function (a, b, z, zpos, zneg, v) {
  C1e <- a <= min(1, 1 - b * min(z) * v^(b - 1), 1 - v^(b -1) * min(z) + min(zpos)/v) & a <= min(1, 1 - b * max(z) * v^(b - 1), 1 - v^(b - 1) * max(z) + max(zpos)/v)
  C1o <- a <= 1 & a > 1 - b * min(z) * v^(b - 1) & a > 1 - b * max(z) * v^(b - 1) & (1 - 1/b) * (b * min(z))^(1/(1 - b)) * (1 - a)^(-b/(1 - b)) + min(zpos) > 0 & (1 - 1/b) * 
    (b * max(z))^(1/(1 - b)) * (1 - a)^(-b/(1 - b)) + max(zpos) >  0
  C2e <- -a <= min(1, 1 + b * v^(b - 1) * min(z), 1 + v^(b - 1) * min(z) - min(zneg)/v) & -a <= min(1, 1 + b * v^(b - 1) * max(z), 1 + v^(b - 1) * max(z) - max(zneg)/v)
  C2o <- -a <= 1 & -a > 1 + b * v^(b - 1) * min(z) & -a > 1 + b * v^(b - 1) * max(z) & (1 - 1/b) * (-b * min(z))^(1/(1 - b)) * (1 + a)^(-b/(1 - b)) - min(zneg) > 0 & (1 - 1/b) * 
...
eht_pll <- function(par,
  ...
  for(ii in seq_along(index)){
    ...
    if(constrain){
      zpos <- c(min(apply(X,1,min) - X0), max(apply(X, 1, max) - X0))
      z <- range((X - X0 * alpha)/(X0^beta))
      zneg <- c(min(apply(X,1,min) + X0), max(apply(X, 1, max) + X0))
      const <- texmex_ConstraintsAreSatisfied(a = alpha, b = beta, z = z, zpos = zpos, zneg = zneg, v = max(X0) + 0.1)
      if(!isTRUE(const)){
       obj[ii] <- -1e20 
      }
    }
